...
```shell
usage: mjobs [-h] [-q QUEUE] [-u USER] [-r] [-a] [-d] [-G USER_GROUP]
             [-g GROUP] [-m HOSTS] [-p] [-f FILTER]
             [job_id ...]
...
optional arguments:
  ...
  -a             Displays information about jobs in all states, including jobs
  ...
  -g GROUP       Displays information about jobs attached to the specified job
                 group.
  -m HOSTS       Displays jobs dispatched to the specified hosts.
  -p             Displays pending jobs, together with the pending reasons that
                 caused each job not to be dispatched during the last dispatch
                 turn.
  -f FILTER      Filter the jobs using the specified regex on the job name or
                 pending reason.
```